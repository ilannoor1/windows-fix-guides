...
def render_commands(commands):
    if not commands:
        return """
        <section>
          <h2>Commands / Steps</h2>
          <p>No command data was supplied for this video.</p>
        </section>
        """
    ...
    for item in commands:
        if isinstance(item, dict):
            purpose = esc(item.get("purpose", "Command"))
            wmic = esc(item.get("wmic", ""))
            powershell = esc(item.get("powershell", ""))
            ...
            rows.append(
                ...
                <tr>
                  <td>Command</td>
                  <td colspan="2"><code>{esc(item)}</code></td>
                </tr>
                """
            )
      ...
      <div class="table-wrap">
        <table>
          <thead>
            <tr>
              <th>Purpose</th>
              <th>Old / Previous Command</th>
              <th>PowerShell / Current Command</th>
            </tr>
          </thead>
...
def render_preview(data):
    ...
    video_schema, article_schema, breadcrumb_schema = (
        build_schema(
            data,
            category
        )
    ...
    if status["page_exists"]:
        classification = "EXISTING PRODUCTION CONTENT — PROTECTED"
    else:
...
def should_render(path, data):
    # The example template intentionally contains placeholders.
    # It must never be rendered as a real preview.
...
def main():
    ...
    if not VIDEOS_DIR.exists():
        print("ERROR: data/videos directory does not exist.")
        return 1
    ...
    for path in files:
        ...
        if not should_render(path, data):
            print(
                f"SKIP: {path.name}"
            )
            continue
